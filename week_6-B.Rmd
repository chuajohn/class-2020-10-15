...
x <- tibble(numb_red = rep(seq(from = 0, to = 1000, by = 1), reps)) %>%
  mutate(paddle_red = map_int(numb_red, ~rbinom(n = 1, size = paddle_size, prob = ./urn_size)))

x %>%
  ggplot(aes(x = paddle_red, y = numb_red)) +
  geom_point(alpha = 0.5) +
  labs(title = "Joint Distribution of Red Beads in Paddle and Urn", x = "Number of Red Beads in Paddle", y= "Number of Red Beads in Urn")
```
...
filter(paddle_red == 10) %>%
  ...
  mutate(prob_red = (total/sum(total)) *100) %>%
  select(-total)

distribution %>%
  ggplot(aes(x=numb_red, y = prob_red)) +
  geom_col()
```
...
distribution %>%
  ...
  summarise(final = weighted.mean(res, prob_red))
